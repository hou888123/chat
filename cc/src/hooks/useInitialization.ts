...
import { getProfile, initializeSession, ProfileResponse, InitializeResponse, setSessionId } from '../services/apiService';
...
export const useInitialization = () => {
  ...
  const data = initializeQuery.data;
  
  // 從 questionSuggest 中提取使用介紹的完整問題對象
  const introductionQuestion = data?.questionSuggest?.find(
    item => item.questionContent.includes('使用介紹')
  );
  
  return {
    isLoading: profileQuery.isLoading || (initializeQuery.isLoading && profileQuery.isSuccess),
    isError: shouldShowFrontendError,
    shouldShowTokenLimitError,
    data,
    introductionQuestion, // 返回完整的使用介紹問題對象
  };
}; 