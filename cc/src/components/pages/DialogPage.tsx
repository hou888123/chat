...
// 導入對話歷史項類型
import { DialogHistoryItem, ConsumptionRecord } from '../utils/SystemResponse';
// 導入機敏資訊檢測函數
...
const DialogPage: React.FC = () => {
          ...
          if (part === '{0}' && urlObjects[0]) {
            return (
              <a 
                ...
                onClick={(e) => {urlObjects[0].urlLink?.includes('cathay') ? location.href = urlObjects[0].urlLink : handleTermsClick(e, urlObjects[0].urlLink)}}
              >
  ...
  useEffect(() => {
    if (initData) {
      ...
      // 保存使用介紹的完整問題對象
      if (introductionQuestion) {
        // 將使用介紹問題對象保存到模型中
        model.setIntroductionText && model.setIntroductionText(introductionQuestion);
      }
      
      // 處理API返回的問題建議
      handleQuestionSuggestions(initData);
    }
  }, [initData, shouldShowTokenLimitError, introductionQuestion]);
    ...
    chatRequest(question)
      .then((response: ChatResponse) => {
        ...
        if (response.requestLimitAmount !== undefined && 
          ...
          if (response.responseMessageObject) {
            ...
            if (displayText) {
              ...
              // 添加系統回應到對話歷史
              model.setDialogHistory(prev => [systemResponse]);
            }
          }
        ...
        if (response.responseMessageObject?.genText) {
          // 檢查是否為 tid 為 "A" 且 period 中有兩個日期的情況
          if (response.responseMessageObject.tid === "A" && 
              response.period?.includes('~') ) {
            
            // 創建消費記錄對象
            const consumptionData: ConsumptionRecord = {
              period: response.period, // 使用唯一的日期
              times: 1, // 假設只有一次消費
              amount: response.amount || 0, // 消費總金額
              highestDate: response.highestDate || "", // 最高額消費日
              highestAmount: response.highestAmount || 0, // 當日總金額
              storeName: "一線", // 顯示"一線"
              hasChart: true, // 顯示線圖
              details: response.details ? 
                response.details.map(detail => ({
                  ...
                  store: "一線", // 使用固定值
                  cardLastFour: "0000", // 使用固定值
                  category: "消費" // 使用固定值
                })) : []
            };
            
            // 創建系統回應對象，使用 API 返回的系統回應文字，並添加消費記錄
            let systemResponse: DialogHistoryItem = {
              type: 'system', 
              text: response.responseMessageObject.errorMessage || response.responseMessageObject.genText,
              withCard: true, // 顯示消費卡片
              consumptionData: consumptionData, // 添加消費記錄
              withFeedback: true, // 預設顯示讚與倒讚
              questionId,
              requestId: response.requestId // 保存 requestId
            };
