...
import ConsumptionRecordComponent from '../consumption/ConsumptionRecord';
...
const ChatContent: React.FC<ChatContentProps> = memo(({
  ...
  // 判斷是否為系統錯誤訊息
  const isSystemErrorMessage = useCallback((text: string) => {
    // 根據文本內容判斷是否為系統錯誤訊息
    return text && (
  ...
  const shouldShowRecommendQuestions = useCallback((item: DialogHistoryItem, isLastMessage: boolean, index: number) => {
    ...
    // 如果是系統錯誤訊息，則不顯示
    if (isSystemErrorMessage(item.text)) return false;
    ...
    // 如果是令牌限制錯誤，則不顯示
    if (item.showGoToAction) return false;
  ...
  const lastUserMessagePosition = lastUserMessageIndex >= 0 ? dialogHistory.length - 1 - lastUserMessageIndex : -1;
  const REASK_OPTIONS = [
    "問時段內的<span class='u-font-bold'>刷卡消費</span>",
    "問時段內的<span class='u-font-bold'>店家消費</span>",
    "問時段內的<span class='u-font-bold'>消費類別</span>",
  ];
  return (
        ...
        if (item.type === 'user') {
          ...
          return (
            <div 
              ...
              style={(isLastMessage ? { minHeight: (item.showGoToAction) ? containerHeight - 46 : lastMessageMinHeight - 16 } : {})}
            >
              <div className='u-w-full'>
                ...
                {/* 對於包含消費卡片的消息，直接顯示API返回的系統回覆文字 */}
                {item.withCard && item.consumptionData && (
                  <>
                    {/* 直接使用item.text顯示API返回的系統回覆文字 */}
                    <SystemResponseMessage
                      text={item.text}
                      typingSpeed={60}
                      onTypingComplete={() => handleTypingComplete(`${messageId}-card`, false)}
                      skipTypingEffect={shouldShowConsumptionCard(messageId)}
                    />
                    
                    {/* 消費總覽模組 - 在消費文字描述顯示完成後才顯示，並添加動畫 */}
                    {shouldShowConsumptionCard(messageId) && (
                      <motion.div
                        initial={{ opacity: 0, y: 30 }}
                        animate={{ opacity: 1, y: 0 }}
                        transition={{ duration: 0.4, ease: "easeOut" }}
                      >
                        <ConsumptionRecordComponent 
                          consumptionRecord={item.consumptionData}
              ...
              {!item.isIntroduction && (
                ...
                !isSystemErrorMessage(item.text) && !(isLastMessage && showSystemError) && item.withFeedback !== false
              ) && (
                <motion.div 
                ...
                >
                  ...
                  {item.showGoToAction ? (
                    ...
                    hideReinquiryButtons ? (
                      ...
                      /* 消費紀錄（白底明細、圖表）模組顯示重新提問按鈕 */
                      item.withCard && item.consumptionData && isLastConsumptionCard ? (
                        <FeedbackModule
                          type="reask"
                          showActionButton={false}
                          expandedOptionsTitle="重新提問"
                          expandedOptions={REASK_OPTIONS}
                          onOptionClick={handleReaskOptionClickInternal}
                          onLike={() => onLike(item.questionId)}
                          onDislikeOptionSelect={(option) => onDislikeOptionSelect(option, item.questionId)}
                          feedbackOptions={item.feedbackOptions || feedbackOptions}
                          iconSources={iconSources}
